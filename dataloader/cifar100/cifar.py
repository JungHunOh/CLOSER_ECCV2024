...
class CIFAR10(VisionDataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None,
                 download=False, index=None, base_sess=None, args=None, session=None):
        ...
        if self.train:
            ...
            self.transform = PretrainTransform('cifar100', args)

            #self.transform = transforms.Compose([
            #    #transforms.RandomCrop(32, padding=4),
            #    transforms.RandomApply([transforms.RandomResizedCrop(32, scale=(0.25,1), ratio=(1,1))], p=0.5),
    ...
    def SelectfromDefault_val(self, data, targets, index):
        data_tmp = []
        targets_tmp = []
        for i in index:
            if i >= 60:
                ind_cl = np.where(i-60 == targets)[0]
            else:
                ind_cl = np.where(i == targets)[0]
            
            selected = data[ind_cl]
            if i >= 60:
                selected = np.rot90(selected,2,axes=(1,2))
            if len(data_tmp)==0:
                data_tmp = selected
                ind_cl = np.where(i == targets)[0]
                targets_tmp = targets[ind_cl]
            else:
                data_tmp = np.vstack((data_tmp, selected))
                ind_cl = np.where(i == targets)[0]
    ...
    def NewClassSelector_val(self, data, targets, index):
        data_tmp = []
        targets_tmp = []
        ...
        for idx, i in enumerate(index):
            cls = idx + (self.session-1)*5
            ind_cl = np.where(cls == targets)[0]
            ind_cl = np.random.choice(ind_cl, len(i), replace=False)

            if len(data_tmp)==0:
                data_tmp = np.rot90(data[ind_cl],2,(1,2))
                cls = targets[i[0]]
                ind_cl = np.where(cls == targets)[0]
                ind_cl = np.random.choice(ind_cl, len(i), replace=False)
                ...
                data_tmp = np.vstack((data_tmp, np.rot90(data[ind_cl],2,(1,2))))
                cls = targets[i[0]]
                ind_cl = np.where(cls == targets)[0]
                ind_cl = np.random.choice(ind_cl, len(i), replace=False)
                targets_tmp = np.hstack((targets_tmp, targets[ind_cl]))
